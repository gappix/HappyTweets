...
/**
	...
	*
	* @param args: String containing an integer number according to Location class options
	*/
class TweetStreamingApp(args: Seq[String]) extends TweetApp("streaming") with Logging {
	...
	override def Run() {
		...
		*----------------------------------------------------*/
		...
		val query = new FilterQuery()
			.locations(Array(9.07683, 45.42503), Array(9.26089, 45.53212))
		...
		val streamTweets = ssc.receiverStream(TwitterReceiver.filter(Util.config(args),query))
		
		
		//TwitterUtils.createStream(ssc, None, filters, StorageLevel.MEMORY_ONLY_SER)
